...
import { createLeaveApplication } from "./actions";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function AddLeaveApplications() {
  ...
  const {
    ...
    formState: { errors },
    setError,
  } = useForm<formFields>({ resolver: zodResolver(leaveTypeSchema) });
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: formFields) => {
      return createLeaveApplication(data);
    },
    onSuccess: (data) => {
      if (data.error) {
        showToast({
          ...
          message:
            data?.error ||
            "An error occurred while creating the leave application",
          type: "error",
        });
        return;
      }
      showToast({
        title: "Success",
        message: data?.message || "Leave application created successfully",
        type: "success",
      });
      setOpen(false);
        ...
        <DialogContent className="sm:max-w-[700px]">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <DialogFooter className="mt-4">
              ...
              <Button type="submit">
                {isPending ? <Spinner /> : "Add Leave Application"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
