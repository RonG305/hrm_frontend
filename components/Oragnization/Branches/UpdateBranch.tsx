...
export function UpdateBranch({branch}: {branch: Branch}) {
  ...
  const {
    ...
    control,
    setError,
  } = useForm<formFields>({ resolver: zodResolver(branchSchema), defaultValues: branch });
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: formFields) => {
      return updateBranch(branch.id, data);
    },
    onSuccess: (data) => {
      showToast({
        title: "Success",
       ...
       <span className="flex gap-x-2"><SquarePenIcon size={18} className="mr-2" />Edit</span>
        ...
        <DialogContent className="sm:max-w-[700px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the details below to update the branch.
            </DialogDescription>
          </DialogHeader>
